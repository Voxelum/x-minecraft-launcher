<template>
  <div
    class="absolute left-0 top-0 flex h-full max-h-full w-full flex-col transition-all"
    :class="{
      'pt-35': compact,
      'pt-47': !compact,
    }"
  >
    ...
    <div
      class="flex h-full flex-col overflow-auto py-0"
      @dragend="onDragEnd"
      @dragover.prevent
    >
      <Hint
      ...
      <v-virtual-scroll
        ...
        :items="items"
        :bench="2"
        class="visible-scroll max-h-full overflow-auto"
        item-height="100"
        @wheel.native="onScroll"
      >
        <template #default="{ item, index }">
          <div class="invisible-scroll mx-5 last:mb-4">
            <ModCard
