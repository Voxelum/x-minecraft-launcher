      ...
      <div class="p-4 flex flex-col gap-3 shrink-0">
        ...
        >
          ...
          <input
            ...
            :placeholder="t('shaderPack.search')"
        ...
        <div class="flex p-1 bg-[#212121] rounded-lg">
          ...
          >
            {{ state === "market" ? t("shaderPack.market") : t("mod.installed") }}
          </div>
        </div>
...
import {
  kInstanceShaderPacks,
  InstanceShaderFile,
} from "@/composables/instanceShaderPack";
...
const onContextMenu = (e: MouseEvent, item: any) => {
  ...
  // Add shader-specific actions for non-group items
  const shaderItems: any[] = [];
  if (!("projects" in item) && item.installed?.length > 0) {
    const installedFile = item.installed[0];
    const isActive = shaderPack.value === installedFile.fileName;

    // Enable/Activate shader
    if (!isActive) {
      shaderItems.push({
        icon: "check_circle",
        text: t("shaderPack.enable"),
        onClick: () => {
          onEnable(installedFile);
        },
      });
    } else {
      // Disable shader (set to empty)
      shaderItems.push({
        icon: "cancel",
        text: t("shaderPack.disable"),
        onClick: () => {
          shaderPack.value = "";
        },
      });
  ...
  const allItems = [...groupItems, ...shaderItems];
...
const curseforgeModCategories = computed(() => {
  ...
  if (!cats) return [];
  const parent = cats.find((c) => c.slug === "shaders");
...
const modrinthModCategories = computed(() => {
  return modrinthTags.value.filter((c) => c.project_type === "shader");
});
...
// Local items (installed shader packs)
...
const onEnable = async (f: InstanceShaderFile | string) => {
  shaderPack.value = typeof f === "string" ? f : f.fileName;
};
