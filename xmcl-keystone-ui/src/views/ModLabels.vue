...
const getDepIcon = (name: string, icon?: string) => {
  if (icon) return icon
  if (name === 'forge') return BUILTIN_IMAGES.forge
  if (name === 'minecraft') return BUILTIN_IMAGES.minecraft
  if (name === 'fabricloader' || name.startsWith('fabric-') || name === 'fabric') return BUILTIN_IMAGES.fabric
  return ''
}
