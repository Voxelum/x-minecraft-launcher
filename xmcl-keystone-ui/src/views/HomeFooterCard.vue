    ...
    >
      <v-card-text class="flex flex-col gap-2 p-3 pt-2">
        <div class="flex gap-4 items-center">
          <div
            class="tabs flex gap-4 items-center"
            :style="{
              '--underline-left': underlineLeft + 'px',
          ...
          <div class="controls" style="margin-right: 0.18rem">
            <v-btn icon small @click="onViewDashboard">
              <v-icon size="20" class="rotate-[45deg]"> unfold_more </v-icon>
            </v-btn>
          </div>
        </div>
        <div
          class="flex flex-col gap-2 transition-all duration-400"
          :style="
            active || dragover
              ? {
                  height: '136px',
                  overflow: selected === 1 || selected === 2 ? 'auto' : 'unset',
                }
              : { height: '4rem' }
          "
        >
          <InstanceServerCard v-if="selected === 2" :instance="instance" />
          <HomeCardListItem
            v-else
            v-for="item in items"
...
import type {
  StoreProjectVersion,
  StoreProjectVersionDetail,
} from "@/components/StoreProjectInstallVersionDialog.vue";
...
const modrinthVersions = useSWRVModel(
  getModrinthVersionModel(
    modrinthProjectId,
    undefined,
    modLoaderType,
    computed(() =>
      minecraftVersion.value ? [minecraftVersion.value] : undefined
    )
  )
);
...
const curseforgeFiles = useSWRVModel(
  getCurseforgeProjectFilesModel(
    curseforgeProjectId,
    minecraftVersion,
    curseforgeModLoaderTypeRef
  )
);
...
): Promise<StoreProjectVersionDetail> {
  if (upstream.value?.type === "modrinth-modpack") {
    const target = (modrinthVersions.data.value || []).find(
      (v) => v.id === version.id
    );
    if (!target) return { changelog: "", dependencies: [], version };

    const projects =
      target.dependencies?.map((v) => v.project_id).filter((v) => !!v) || [];
    const lookup = Object.fromEntries(
      target.dependencies?.map((p) => [p.project_id, p.dependency_type]) || []
    );

    try {
      const { clientModrinthV2 } = await import("@/util/clients");
      const matched =
        projects.length > 0 ? await clientModrinthV2.getProjects(projects) : [];
    ...
    const target = (curseforgeFiles.data.value?.data || []).find(
      (v) => v.id.toString() === version.id
    );
    if (!target) return { changelog: "", dependencies: [], version };
    ...
    try {
      ...
      const lookup = Object.fromEntries(
        target.dependencies?.map((p) => [p.modId, mapping[p.relationType]]) ||
          []
      );
      const detail =
        target.dependencies && target.dependencies.length > 0
          ? await clientCurseforgeV1.getMods(
              target.dependencies.map((d) => d.modId)
            )
          : [];
      ...
      if (curseforgeProjectId.value) {
        try {
          const changelogHtml = await clientCurseforgeV1.getModFileChangelog(
            curseforgeProjectId.value,
            target.id
          );
...
const underlineLeft = computed(() => {
  ...
  // For server tab (selected === 2)
  const newsWidth = newsRef.value?.offsetWidth || 0;
  return contentWidth + 16 + newsWidth + 16;
});
...
const { install: installResourcePack } = useService(
  InstanceResourcePacksServiceKey
);
...
const { install: installShaderPack } = useService(
  InstanceShaderPacksServiceKey
);
...
async function onInstallVersion(v: StoreProjectVersion) {
  ...
  try {
    ...
    const [modpack] = await installModapckFromMarket(
      upstream.value?.type === "curseforge-modpack"
        ? {
            market: 1,
...
const items = computed(() => {
  if (selected.value === 0) {
    return [
      ...
      },
      {
        ...
        text: dragover.value
          ...
          : t("resourcepack.enable", {
              count: enabledResourcePacks.value.length,
            }),
        highlighted: false,
      ...
      },
      {
        ...
        text: dragover.value
          ? t("save.dropHint")
          : t("save.createdWorlds", { count: savesLength.value }),
        highlighted: false,
    ...
    return latestVersions.value.map((v) => {
      const version: StoreProjectVersion =
        "name" in v
          ? {
              id: v.id,
              name: v.name,
      ...
      return {
        ...
        highlighted:
          (upstream.value?.type === "modrinth-modpack" &&
            upstream.value.versionId === v.id) ||
          (upstream.value?.type === "curseforge-modpack" &&
            upstream.value.fileId === v.id),
        install: () => {
