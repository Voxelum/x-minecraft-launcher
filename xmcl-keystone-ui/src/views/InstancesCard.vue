<template>
  ...
  >
    <v-list-item :color="isSelected ? 'primary' : ''" outlined>
      ...
      <v-list-item-content>
        ...
        <v-list-item-subtitle v-if="description">
          <TextComponent
            :source="
              typeof instance.description === 'object'
    ...
    <v-card-actions class="flex w-full items-center">
      <v-chip outlined label :input-value="false">
        <v-icon left> update </v-icon>
        {{ getDateString(instance.lastAccessDate, { dateStyle: "long" }) }}
      </v-chip>
      <InstanceServerCard :instance="instance" />

      <div class="flex-grow" />
      <v-btn icon @click.stop="onSettingClick">
...
const name = computed(
  () => props.instance.name || `Minecraft ${props.instance.runtime.minecraft}`
);
...
const image = computed(() => {
  if (
    status.value.favicon &&
    status.value.favicon !== BuiltinImages.unknownServer
  ) {
...
const getContextMenuItems = useInstanceContextMenuItems(
