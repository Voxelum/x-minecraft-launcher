<template>
  ...
  >
    ...
    <div v-if="autoHide" class="sidebar-trigger" :class="triggerClasses" />

    <div class="sidebar-notch" :class="sidebarClasses" :style="wrapperStyles">
      <div
        ref="container"
      ...
      >
        ...
        <AppSideBarNotchItem
          ...
          :icon-size="iconSize"
          :tooltip="
            () => ({ text: t('store.name', 2), direction: tooltipDirection })
          "
          to="/store"
        />
        ...
        <div class="sidebar-notch__instances"></div>
        ...
        <AppSideBarNotchItem
          :icon-size="iconSize"
          :tooltip="
            () => ({ text: t('setting.name', 2), direction: tooltipDirection })
          "
          to="/setting"
        >
          <v-badge right overlap :value="state?.updateStatus !== 'none'">
            ...
            <v-icon class="sidebar-notch-item__icon" :size="iconSize"
              >settings</v-icon
            >
          </v-badge>
        </AppSideBarNotchItem>
...
const instanceItems = computed(() => {
  ...
  const getAccessTime = (item: (typeof groups.value)[0]): number => {
    if (typeof item === "string") {
      ...
      return Math.max(
        ...item.instances.map(
          (path) => instanceMap.get(path)?.lastAccessDate ?? 0
        )
      );
    }
  };
