<template>
  <div>
    <!-- Server Card (shown when instance has server configured) -->
    <div v-if="instance?.server?.host" class="mb-4">
      <HomeServerCard :instance="instance" />
    </div>
...
import HomeServerCard from "@/components/InstanceServerCard.vue";
  ...
  () => ({
    md: [
      { x: 0, y: 0, w: 3, h: 9, minW: 2, minH: 4, i: rawType(CardType.Mod) },
      {
        ...
        minW: 2,
        minH: 4,
        i: rawType(CardType.ResourcePack),
      },
      ...
      },
      {
        ...
        minW: 3,
        minH: 4,
        i: rawType(CardType.Screenshots),
      },
    ],
    lg: [
      { x: 0, y: 0, w: 3, h: 9, minW: 2, minH: 4, i: rawType(CardType.Mod) },
...
const onResized = (
  ...
  if (Number(i) === CardType.Screenshots) {
    if (!screenshotItem) {
      screenshotItem = document
        .getElementsByClassName("screenshot-item")
...
const getRowCount = (width: number) =>
  width ? Math.floor((width - 34) / 30) : 7;
const resourcePackRowCount = computed(() =>
  getRowCount(containerWidths[CardType.ResourcePack])
);
const modRowCount = computed(() => getRowCount(containerWidths[CardType.Mod]));
const saveRowCount = computed(() =>
  getRowCount(containerWidths[CardType.Save])
);
</script>
