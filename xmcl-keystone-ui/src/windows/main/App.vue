<template>
  ...
  >
    ...
    <div class="relative flex h-full overflow-auto" :class="layoutClasses">
      <AppSideBarClassic v-if="sidebarStyle === 'classic'" />
      <AppSideBarNotch v-else />
      <main
        class="relative flex max-h-full flex-1 flex-col overflow-auto"
        :class="mainClasses"
      >
...
import {
  kSidebarSettings,
  useInjectSidebarSettings,
  useSidebarSettings,
} from "@/composables/sidebarSettings";
...
import {
  kInstanceLauncher,
  useInstanceLauncher,
} from "@/composables/instanceLauncher";
...
const layoutClasses = computed(() => ({
  "flex-row":
    sidebarPosition.value === "left" || sidebarPosition.value === "right",
  "flex-col":
...
watch(
  dark,
  (newVal) => {
    state.value?.themeSet(
      newVal === "system" ? newVal : newVal ? "dark" : "light"
    );
  },
  { immediate: true }
);
