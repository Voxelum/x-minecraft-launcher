...
describe('marketRoute', () => {
  test('routes Modrinth content urls to their launcher pages', () => {
    ...
    expect(getMarketRoutePathFromModrinthUrl(new URL('https://modrinth.com/resourcepacks/translations-for-sodium'))).toBe('/resourcepacks')
    expect(getMarketRoutePathFromModrinthUrl(new URL('https://modrinth.com/shaders/complementary-reimagined'))).toBe('/shaderpacks')
  })
