...
import { isRuntimeServiceError } from '@/telemetry'

export function useExceptionHandler<T extends ExceptionBase>(
  type: ExceptionConstructor<Exception<T>>,
  handler: (e: T) => void,
) {
...
export function useExceptionHandlers() {
  const exceptionHandlers: Record<
    string,
    [ExceptionConstructor<Exception<any>>, Array<(e: unknown) => void>]
  > = {}
  const serviceErrorHandlers: Record<
    string,
    Array<(e: any, serviceName: string, serviceMethod: string) => void>
  > = {}
  const errorHandlers: Array<(e: unknown) => boolean> = []

  window.addEventListener('unhandledrejection', (ev) => {
    const runtimeServiceError = isRuntimeServiceError(ev.reason)
    const exHandler = exceptionHandlers[ev.reason.name]
    if (!runtimeServiceError) {
      console.log(`Handle exception ${ev.reason.name} from ${ev.reason.serviceName}`)
    }
    const servHandler = ev.reason?.serviceName
      ? serviceErrorHandlers[ev.reason.serviceName]
      : undefined
...
export const kExceptionHandlers: InjectionKey<ReturnType<typeof useExceptionHandlers>> =
