  ...
  <v-card class="rounded-xl w-full">
    <v-card-title>
      <div class="flex items-center gap-2">
        <span class="font-bold text-sm uppercase text-gray-300">
          {{ title }}
        </span>
    ...
    <v-card-text>
      <div class="grid grid-cols-4 lg:(grid-cols-2) gap-2 pt-2">
        <div
          ...
          class="flex items-center gap-2 p-2 rounded-lg cursor-pointer transition hover:bg-white/10"
          :class="isSelected(item.id) ? 'bg-white/8' : 'text-gray-400'"
