<template>
  <section
    ...
    class="commercial-account-panel surface-panel"
  ...
  >
    ...
    <template v-if="account">
      ...
      <div class="mt-5">
        <div class="text-sm font-semibold">{{ t('commercialAccount.loginMethods') }}</div>
        <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <div
            v-for="identity in identities"
            :key="identity.provider"
            class="surface-card-row rounded-xl p-3 flex items-center gap-3"
          >
            <v-icon aria-hidden="true">{{ providerIcon(identity.provider) }}</v-icon>
            <div class="min-w-0 flex-grow">
              <div class="text-sm font-medium">{{ providerName(identity.provider) }}</div>
              <div class="truncate text-xs opacity-60">{{ identity.displayName || t('commercialAccount.linked') }}</div>
            </div>
            <v-btn
              ...
              size="x-small"
              variant="text"
              :aria-label="t('commercialAccount.unlink', { provider: providerName(identity.provider) })"
              :disabled="busy"
              @click="unlinkIdentity(identity.provider)"
            >
              <v-icon size="16" aria-hidden="true">link_off</v-icon>
            </v-btn>
          </div>
        </div>
      </div>
      ...
      <div class="mt-5 flex flex-wrap items-center gap-2">
        <v-btn
          v-if="account.status === 'deletion_pending'"
          color="warning"
          variant="tonal"
          :loading="busy"
          @click="cancelDeletion"
        >
          {{ t('commercialAccount.cancelDeletion') }}
        </v-btn>
        <v-btn
          v-else
          color="error"
          variant="text"
          :disabled="busy"
          @click="deletionDialogShown = true"
        >
          {{ t('commercialAccount.requestDeletion') }}
        </v-btn>
      </div>
    </template>
    ...
    <template v-else>
      <div class="mt-5">
        <div class="text-sm font-semibold">{{ t('commercialAccount.signInWith') }}</div>
        <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            <v-icon start aria-hidden="true">xmcl:microsoft</v-icon>
            Microsoft
          </v-btn>
          <v-btn
          ...
          >
            <v-icon start aria-hidden="true">xmcl:modrinth</v-icon>
            Modrinth
          </v-btn>
          <v-btn
    ...
    <SimpleDialog
      v-model="deletionDialogShown"
      :title="t('commercialAccount.requestDeletion')"
      :confirm="t('commercialAccount.requestDeletion')"
      color="error"
      @confirm="confirmDeletion"
    >
      {{ t('commercialAccount.deletionWarning') }}
    </SimpleDialog>
  </section>
</template>
