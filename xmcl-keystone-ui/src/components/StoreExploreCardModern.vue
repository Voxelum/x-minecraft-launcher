  ...
  >
    ...
    <div class="aspect-[16/9] relative rounded-2xl overflow-hidden bg-black/20 z-1">
      ...
      <div class="absolute top-3 right-3 flex gap-2">
        <v-chip x-small color="black" class="bg-opacity-60 backdrop-blur-md font-bold uppercase tracking-wider text-3">
          {{ value.type }}
        </v-chip>
      </div>
    ...
    <div class="p-4 flex flex-col flex-1 gap-2">
      <div class="flex items-start gap-3">
        ...
        <div class="overflow-hidden">
          <h3 class="font-bold text-base leading-tight truncate text-white group-hover:text-primary transition-colors" v-shared-tooltip="value.title">
            {{ value.title }}
          </h3>
          <p class="text-xs text-gray-500 mt-0.5 truncate">{{ value.author }}</p>
        </div>
      </div>

      <p class="text-xs text-gray-400 line-clamp-2 h-9 leading-relaxed">{{ value.description }}</p>

      <div class="mt-auto pt-2 border-t border-white/5 flex items-center justify-between text-3 text-gray-500 font-medium">
