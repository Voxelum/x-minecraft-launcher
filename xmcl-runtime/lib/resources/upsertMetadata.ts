...
export async function upsertMetadata(metadata: ResourceMetadata, uris: string[], icons: string[], name: string, sha1: string, context: ResourceContext) {
  ...
  const data = await context.db.transaction().execute(async (trx) => {
    ...
    const _resource = await trx
      ...
      .returningAll()
      .executeTakeFirst()
    const _uris = uris.length > 0 ? await trx.insertInto('uris').values(uris.map(u => ({ uri: u, sha1 }))).onConflict(oc => oc.column('uri').doNothing()).returningAll().execute() : []
    const _icons = icons.length > 0 ? await trx.insertInto('icons').values(icons.map(i => ({ icon: i, sha1 }))).onConflict(oc => oc.column('icon').doNothing()).returningAll().execute() : []
    return {
      name,
      ..._resource,
