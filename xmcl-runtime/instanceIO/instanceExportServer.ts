...
export abstract class InstanceExporter {
  ...
  abstract emitFile(path: string, content: string): Promise<void>
  ...
  async exportInstance(options: LaunchOptions, files: InstanceFile[]): Promise<void> {
    ...
    await Promise.all(serverVersion.libraries.map(lib => this.copyFile(mc.getLibraryByPath(lib.path), `libraries/${lib.path}`)))

    // copy all files to the output folder
    await Promise.all(files.map(file => this.copyFile(join(options.gameDirectory, 'server', file.path), file.path)))

    // copy mc server jar
    const serverJarPath = mc.getVersionJar(serverVersion.minecraftVersion, 'server')
    await this.copyFile(serverJarPath, relative(mc.root, serverJarPath))

    if (ops.serverExectuableJarPath) {
      // copy to the output folder
      await this.copyFile(ops.serverExectuableJarPath, 'server.jar')
    ...
    await this.emitFile('server.bat', batContent)
    await this.emitFile('server.sh', shContent)
  }
}
...
export class FSInstanceExporter extends InstanceExporter {
  ...
  copyFile(from: string, to: string): Promise<void> {
    return copy(from, join(this.outputFolder, to))
  }

  emitFile(path: string, content: string): Promise<void> {
    return writeFile(join(this.outputFolder, path), content)
  }
}
