...
describe('CommercialAccountApi', () => {
  it('uses the injected XMCL API origin', async () => {
    ...
    const api = new CommercialAccountApi(fetch, 'https://xmcl-web-api.cijhn.workers.dev/')

    await api.launcherExchange('modrinth', 'provider-secret')

    expect(String(fetch.mock.calls[0]![0]))
      .toBe('https://xmcl-web-api.cijhn.workers.dev/v1/auth/modrinth/launcher-exchange')
  })
