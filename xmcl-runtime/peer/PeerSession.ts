...
import { PeerSessionConnector } from './PeerSessionConnector'
...
/**
 * Represent a session with a peer
 */
export class PeerSession {
  ...
  #activeConnection: RTCPeerConnection | undefined
  ...
  #duplexPool: RTCDuplexChannel[] = []

  #connector: PeerSessionConnector

  #getOrCreateDownloadChannel() {
    if (!this.#activeConnection) {
      throw new ReferenceError('No active connection')
    }
    let idel: RTCDuplexChannel | undefined
    for (const channel of this.#duplexPool) {
      if (!channel.isBusy) {
        idel = channel
        break
      }
    ...
    const channel = new RTCDuplexChannel(this.#activeConnection.createDataChannel(`download-${this.#duplexPool.length}`, {
      ordered: true,
      protocol: 'download',
    }), this.createStream, this.#activeConnection.sctp?.maxMessageSize ?? 16 * 1024)
    this.#duplexPool.push(channel)
    return channel
  }
  ...
  constructor(
    ...
    this.#connector = new PeerSessionConnector(id, context)
    this.#run()
    ...
    while (!this.isClosed && (!this.#activeConnection || this.#activeConnection.connectionState !== 'connected')) {
      await this.#connector.connect().then((c) => {
        if (c && c.channel) {
          ...
          this.context.onConnectionEstablished(this.id, c.connection)
        }
      })
    }
  }
  ...
  createStream = (filePath: string) => {
    ...
    const maxChunk = this.#activeConnection.sctp?.maxMessageSize ?? 16 * 1024
  ...
  setRemoteDescription(remote: RTCPeerConnectionData[]) {
    if (this.#activeConnection) {
      return
    }
    return this.#connector.setRemoteDescription(remote)
  }
  ...
  send<T>(type: MessageType<T>, data: T) {
    if (this.#activeConnection && this.#activeConnection.connectionState !== 'connected' || !this.#channel || this.#channel.readyState !== 'open') {
      return
    }
