...
create table if not exists public.offline_accounts (
  ...
  skin_model text not null default 'steve'
    check (skin_model in ('steve', 'slim')),
...
create index if not exists offline_sessions_token_hash_idx
  on public.offline_sessions(token_hash);

create index if not exists offline_sessions_expiry_idx
  on public.offline_sessions(expires_at);
