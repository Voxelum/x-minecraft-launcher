<template>
  <v-list-tile
    :key="source.version"
    :class="{
      grey: selected === source.version, 'darken-1': selected === source.version,
      'elevation-2': selected === source.version,
    }"
    ripple
    @click="select(source)"
  >
    ...
    <v-list-tile-action style="justify-content: flex-end;">
      <v-chip v-if="source.stable" label color="green">stable</v-chip>
      <v-chip v-else label>unstable</v-chip>
    </v-list-tile-action>
  </v-list-tile>
</template>
