      ...
      <v-window v-model="step">
        <v-window-item :value="0">
          ...
          <v-list style="width: 100%" class="grey darken-4" dark>
            ...
            >
              <v-list-tile-content>
                <v-list-tile-title>1. {{ $t('diagnosis.missingJava.switch', { version: javaIssue.version }) }}</v-list-tile-title>
                <v-list-tile-sub-title>{{ disableUseExistedJava ? $t('diagnosis.missingJava.switch.disabled', { version: javaIssue.version }) : $t('diagnosis.missingJava.switch.message', { version: javaIssue.version }) }}</v-list-tile-sub-title>
              </v-list-tile-content>
              <v-list-tile-action>
                <v-icon>build</v-icon>
              </v-list-tile-action>
            ...
            <v-list-tile ripple @click="downloadAndInstallJava">
              <v-list-tile-content>
                ...
                <v-list-tile-sub-title>{{ $t('diagnosis.missingJava.autoDownload.message', { version: javaIssue.version }) }}</v-list-tile-sub-title>
              </v-list-tile-content>
              <v-list-tile-action>
...
<script lang=ts>
import { computed, defineComponent, inject, reactive, toRefs } from '@vue/composition-api'
...
export default defineComponent({
  ...
  setup() {
    ...
    const disableUseExistedJava = computed(() => validJava.value === undefined && downloadingJava.value)
    const data = reactive({
      step: 0,
    })
