<template>
  <v-combobox
    ...
    :value="value"
    multiple
    :search-input.sync="creating"
    :hide-no-data="!creating"
    @input="byPassInput"
  >
    <template v-slot:no-data>
      <v-list-tile>
        <span class="subheading">{{ createHint }}</span>
        <v-chip label small>{{ creating }}</v-chip>
      </v-list-tile>
    </template>
...
<script lang=ts>
import { defineComponent, reactive, toRefs, remove as $remove } from '@/vue';
  ...
  setup(props, context) {
    ...
    return {
      ...
      deleteItem(item: Item) {
        $remove(data.items, data.items.indexOf(item));
      },
    };
  },
});
