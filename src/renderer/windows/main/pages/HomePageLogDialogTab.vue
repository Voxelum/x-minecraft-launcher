  ...
  <v-tab-item>
    <div style="min-height: 420px; max-height: 420px; overflow: auto; background: #424242">
      <transition name="fade-transition" mode="out-in">
        <v-list v-if="content === '' && files.length !== 0" :key="0">
          <v-list-tile v-for="i in files" :key="i" v-ripple :disabled="loading" avatar @click="openFile(i)">
            <v-list-tile-avatar>
              <v-icon>
                clear_all
              </v-icon>
            ...
            <v-list-tile-action>
              <v-btn icon color="white" flat @click.prevent.stop="showFile(i)">
        ...
        <div v-else-if="content === '' && files.length === 0" style="height: 420px">
