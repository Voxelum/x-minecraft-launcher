<template>
  <v-dialog v-model="isShown" :width="300" :persistent="true">
    <v-card>
...
import { useLaunch, useDialogSelf, useI18n, useIssues } from '@/hooks';

export default {
  setup() {
    const { launch } = useLaunch();
    const { issues } = useIssues();
    const { isShown, showDialog, closeDialog } = useDialogSelf('launch-blocked');
    return {
      status,
      isShown,
      launch,
