  ...
  <v-dialog v-model="isShown" width="550">
    ...
    <v-card>
      <transition name="fade-transition" mode="out-in">
        <stepper-user-service v-if="addingService" :modify="loading" @cancel="addingService=false" />
          ...
          <v-list two-line>
            <template v-for="(s) in services">
              <v-list-tile :key="s.value">
                ...
                <v-list-tile-action>
                  <v-btn :disabled="s.value === 'mojang'" flat icon color="red" @click="remove(s)">
                    <v-icon>delete</v-icon>
                  </v-btn>
                </v-list-tile-action>
              </v-list-tile>
            </template>
            <v-list-tile key="0" ripple @click="newOrEdit(undefined)">
              <v-list-tile-action>
                <v-icon>add</v-icon>
              </v-list-tile-action>
              <v-list-tile-content>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { isShown, hide } = useDialog('user-service');
    const data = reactive({
      editingService: -1,
      addingService: false,
      authOrProfile: 0,
