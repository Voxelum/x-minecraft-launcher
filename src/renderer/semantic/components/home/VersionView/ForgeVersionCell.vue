<template>
    <tr style="cursor: pointer;" :url="meta.url" :version-id='meta.id' @click="$emit('select', meta)">
        <td>
            <div class="ui ribbon label">{{meta.type}}</div>
            <br> {{meta.version}}
            <i v-if="selected" class="ui green check icon"></i>
        </td>
        <td>{{new Date(meta.modified * 1000).toUTCString()}}</td>
...
export default {
    computed: {
        status() {
            return this.$store.state.versions.forge.status[this.meta.version];
        },
        downloadIcon() {
            return {
