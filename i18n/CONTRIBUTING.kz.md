### Технологиялар мен архитектура

Жобада қолданылатын негізгі құралдар:

- [Node.js >=18.17.0](https://nodejs.org/): Негізгі модульдердің жұмыс ортасы.
- [Electron 27](https://electron.atom.io): Жұмыс үстелі қосымшасының платформасы.
- [pnpm](https://pnpm.io/): Монорепозиторий пакет менеджері.
- [TypeScript](https://www.typescriptlang.org/): Жобаның негізгі бағдарламалау тілі.
- [Vue & Vite](https://vuejs.org/): Пайдаланушы интерфейсі мен құрастыру жүйесі.
...
1. **Ішкі модульдермен бірге клондау**:
   ```bash
...
2. **Тәуелділіктерді орнату**:
   ...
   pnpm install
   ```
3. **Әзірлеу режимін іске қосу**:
   ```bash
   pnpm dev
   ```
4. **Типтер мен код стилін тексеру**:
   ```bash
   pnpm check
   pnpm lint
   ```
