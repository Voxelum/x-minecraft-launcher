...
- ⚡️ **Быстрое скачивание**. переиспользование сокетов через HTTP/HTTPS-агенты и параллельная загрузка файлов частями.
- 💻 **Кросс-платформенный**. Лаунчер создан на базе Electron, и поддерживает 🗔 Windows 10/11, 🍎 MacOS, и 🐧 Linux.
- 📚 **Мульти-экземпляры**. Пользователи могут создавать несколько экземпляров, чтобы изолировать различные версии, модификации и настройки запуска.
- 🗂 **Управляйте всеми ресурсами**. Используйте (заданные/переменные) ссылки для установки ресурсов в отдельных случаях, чтобы оптимально использовать диск. Никаких копий модов повсюду! 😆
...
Для основной разработки, смотрите [Помочь в разработке](i18n/CONTRIBUTING.ru.md)

Для разработки локализации i18n, пожалуйста, перейдите по ссылке [Начало работы с локализацией](https://docs.xmcl.app/en/guide/i18n)
