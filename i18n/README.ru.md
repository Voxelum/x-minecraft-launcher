...
Если у вас установлен winget, вы можете установить программу командой:

```bash
winget install CI010.XMinecraftLauncher
```

Установка через Homebrew (macOS tap):
...
В Linux XMCL также доступен на Flathub:
...
- 📥 **Автоматическая загрузка и дополнение**. Поддержка загрузки `Minecraft`, `Forge`, `Fabric`, `Quilt`, `OptiFine`, `JVM` с официальных и сторонних зеркал.
- ⚡️ **Высокая скорость загрузки**. Повторное использование соединений через HTTP/HTTPS-агенты и параллельная загрузка файлов частями.
- 💻 **Кроссплатформенность**. Лаунчер создан на базе Electron и поддерживает 🗔 Windows 10/11, 🍎 macOS и 🐧 Linux.
- 📚 **Управление несколькими инстансами (профилями)**. Возможность создания изолированных профилей для различных версий игры, модов и параметров запуска.
- 🗂 **Эффективное управление ресурсами**. Использование жестких и символических ссылок для установки ресурсов без дублирования файлов и экономии места на диске! 😆
- 🔥 **Встроенная интеграция CurseForge и Modrinth**. Поиск и загрузка ресурсов прямо внутри лаунчера.
- 📦 **Импорт и экспорт модпаков**. Полная совместимость с официальными форматами модпаков CurseForge и Modrinth!
- 🔒 **Поддержка нескольких аккаунтов**. Встроенная авторизация через Microsoft и Mojang Yggdrasil API, поддержка [ely.by](https://ely.by/) и [littleskin.cn](https://littleskin.cn), а также возможность подключения сторонних серверов аутентификации!
- 🔗 **P2P мультиплеер (одноранговая сеть)**. Играйте с друзьями по виртуальной локальной сети через интернет без сложной настройки!
- 🔑 **Цифровая подпись и современные пакеты**. В Windows поддерживаются пакеты `appx` и `appinstaller` — никаких предупреждений от браузера или экранов SmartScreen! 😎

## Библиотеки ядра (Core Libraries)

Этот репозиторий также включает в себя **Minecraft Launcher Core** (пакеты `@xmcl/*`) — набор npm-пакетов, предоставляющих функции для создания лаунчера Minecraft. [Документация API](https://docs.xmcl.app/en/core)
...
| [@xmcl/installer](../packages/installer) | Установка Minecraft, Forge, Fabric, Quilt, OptiFine, JVM | [![npm](https://img.shields.io/npm/v/@xmcl/installer.svg)](https://www.npmjs.com/package/@xmcl/installer) |
| [@xmcl/user](../packages/user) | Аутентификация пользователей и скины | [![npm](https://img.shields.io/npm/v/@xmcl/user.svg)](https://www.npmjs.com/package/@xmcl/user) |
| [@xmcl/mod-parser](../packages/mod-parser) | Парсинг модов Forge/LiteLoader/Fabric | [![npm](https://img.shields.io/npm/v/@xmcl/mod-parser.svg)](https://www.npmjs.com/package/@xmcl/mod-parser) |
...
| [@xmcl/nbt](../packages/nbt) | Парсинг NBT | [![npm](https://img.shields.io/npm/v/@xmcl/nbt.svg)](https://www.npmjs.com/package/@xmcl/nbt) |
| [@xmcl/game-data](../packages/game-data) | Загрузка данных уровней или servers.dat | [![npm](https://img.shields.io/npm/v/@xmcl/game-data.svg)](https://www.npmjs.com/package/@xmcl/game-data) |
| [@xmcl/resourcepack](../packages/resourcepack) | Парсинг ресурс-паков | [![npm](https://img.shields.io/npm/v/@xmcl/resourcepack.svg)](https://www.npmjs.com/package/@xmcl/resourcepack) |
| [@xmcl/gamesetting](../packages/gamesetting) | Парсинг настроек игры | [![npm](https://img.shields.io/npm/v/@xmcl/gamesetting.svg)](https://www.npmjs.com/package/@xmcl/gamesetting) |
| [@xmcl/client](../packages/client) | Сетевые утилиты клиента Minecraft | [![npm](https://img.shields.io/npm/v/@xmcl/client.svg)](https://www.npmjs.com/package/@xmcl/client) |
| [@xmcl/model](../packages/model) | Отображение 3D-моделей игроков и блоков | [![npm](https://img.shields.io/npm/v/@xmcl/model.svg)](https://www.npmjs.com/package/@xmcl/model) |
| [@xmcl/text-component](../packages/text-component) | Парсинг текстовых компонентов Minecraft | [![npm](https://img.shields.io/npm/v/@xmcl/text-component.svg)](https://www.npmjs.com/package/@xmcl/text-component) |
| [@xmcl/forge-site-parser](../packages/forge-site-parser) | Парсинг сайта Forge | [![npm](https://img.shields.io/npm/v/@xmcl/forge-site-parser.svg)](https://www.npmjs.com/package/@xmcl/forge-site-parser) |
| [@xmcl/file-transfer](../packages/file-transfer) | Высокопроизводительная загрузка файлов | [![npm](https://img.shields.io/npm/v/@xmcl/file-transfer.svg)](https://www.npmjs.com/package/@xmcl/file-transfer) |
| [@xmcl/nat-api](../packages/nat-api) | Проброс портов UPnP и NAT-PMP | [![npm](https://img.shields.io/npm/v/@xmcl/nat-api.svg)](https://www.npmjs.com/package/@xmcl/nat-api) |
...
| [@xmcl/unzip](../packages/unzip) | Обертка распаковки zip (yauzl) | [![npm](https://img.shields.io/npm/v/@xmcl/unzip.svg)](https://www.npmjs.com/package/@xmcl/unzip) |
...
| [@xmcl/bytebuffer](../packages/bytebuffer) | Реализация ByteBuffer | [![npm](https://img.shields.io/npm/v/@xmcl/bytebuffer.svg)](https://www.npmjs.com/package/@xmcl/bytebuffer) |

## Руководство по внесению вклада

[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/Voxelum/x-minecraft-launcher)

Для разработчиков: смотрите инструкцию по [Вкладу](CONTRIBUTING.ru.md).

Для переводчиков и локализации (i18n): пожалуйста, перейдите по ссылке [Начало работы с локализацией](https://docs.xmcl.app/en/guide/i18n).
...
| [![](https://github.com/DGP-Studio/Snap.Hutao/assets/10614984/73ae8b90-f3c7-4033-b2b7-f4126331ce66)](https://signpath.io/) | Бесплатная подпись кода для Windows предоставлена [SignPath.io](https://signpath.io/), сертификат от [SignPath Foundation](https://signpath.org/) |
| :----------------------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------: |
| [![](https://deno.com/images/deno_logo_4.gif)](https://deno.com/deploy) | [Deno Deploy](https://deno.com/deploy), XMCL использует платформу Deno для serverless JavaScript приложений. Предоставлено [Deno](https://deno.com/) |
| [![](../assets/EdgeOne.png)](https://edgeone.ai/) | [Tencent EdgeOne](https://edgeone.ai/) — ускорение CDN и защита проекта спонсируются Tencent EdgeOne. |
...
### 🌍 Сообщество и локализация

**[BANSAFAn/Baneronetwo](https://github.com/BANSAFAn)**
Поддержка и модерация сообщества RU/UK
