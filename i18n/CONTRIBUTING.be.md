### Тэхналагічны стэк і архітэктура

Агляд асноўных інструментаў, якія выкарыстоўваюцца ў праекце:

- [Node.js >=18.17.0](https://nodejs.org/): Асяроддзе выканання для асноўных бібліятэк.
- [Electron 27](https://electron.atom.io): Дэсктопная платформа прыкладання.
- [pnpm](https://pnpm.io/): Пакетны менеджэр монорэпазіторыя.
- [TypeScript](https://www.typescriptlang.org/): Асноўная мова праграмавання.
- [Vue & Vite](https://vuejs.org/): Карыстальніцкі інтэрфейс і сістэма зборкі.
...
1. **Кланаваць рэпазіторый з субмодулямі**:
   ```bash
...
2. **Усталяваць залежнасці**:
   ...
   pnpm install
   ```
3. **Запусціць рэжым распрацоўкі**:
   ```bash
   pnpm dev
   ```
4. **Праверка тыпаў і лінтар**:
   ```bash
   pnpm check
   pnpm lint
   ```
